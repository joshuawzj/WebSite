...
public class DownloadBill
{
    ...
    {
        LogHelper.Log("DownloadBill is processing...");

        WxPayData data = new WxPayData();
        data.SetValue("bill_date", bill_date);//账单日期
        data.SetValue("bill_type", bill_type);//账单类型
        WxPayData result = WxPayApi.DownloadBill(data, 6);//提交下载对账单请求给API，接收返回结果

        LogHelper.Log("DownloadBill process complete, result : " + result.ToXml());
