...
public partial class Ajax_AddOrder : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (WebUser.IsLogin())
        {
            ...
            if (dataTable.Rows.Count > 0)
            {
                ...
                //支付金额
                decimal total_fee = dataTable.Rows[0]["price"].ToDecimal();
                //聚合支付商户号
                ...
                if (primaryValue > 0)
                {
					...
					LogHelper.Log("下单结果 : " + result);
					
                    JObject jo = (JObject)JsonConvert.DeserializeObject(result);
                    int code = jo["code"].ToInt32();
                    string msg = jo["msg"].ToStr();
                    if (code == 0)
                    {
						//下单成功，返回二维码图片
                        output = "{\"info\":\"" + jo["data"].ToStr() + "\",\"status\":\"y\"}";
                    }
                    else
                    {
                        output = "{\"info\":\"" + msg + "\",\"status\":\"n\"}";
                    }
                }
                else
                {
