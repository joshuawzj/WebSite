 ...
 *模板路径：C:\wwwroot\WebSite\cn\template\修改密码.shtml
...
<script>
    function send() { 
        ...
        $.post("<%=Whir.ezEIP.Web.SysManagePageBase.Insten.AppName %>ajax/AjaxData.aspx", { action: "sendcodeModify" }, function (ref) {
            if (ref == "1") {
    ...
    function sava() {
        ...
        $.post("<%=Whir.ezEIP.Web.SysManagePageBase.Insten.AppName %>ajax/AjaxData.aspx", { action:"modify", code, pwd,renewpassword }, function (ref) {
            if (ref == "1") {
                 alert("修改成功");
            } else {
                alert(ref);
