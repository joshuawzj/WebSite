...
public partial class label_survey_submitsurvey : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            string content = RequestUtil.Instance.GetQueryString("content");//调查内容
            int columnId = RequestUtil.Instance.GetQueryInt("columnid", 0);//问题调查栏目ID
            string Ids = RequestUtil.Instance.GetQueryString("ids").Trim(',');

            string SurveyTableName = "";//主题表名
            ...
            if (Ids.Length > 0 && TopicID != 0)
            {
                ...
                string SQLSurvey = "SELECT * FROM {0} WHERE {0}_PID=@0".FormatWith(SurveyTableName);
                DataTable dtSurvey = DbHelper.CurrentDb.Query(SQLSurvey, TopicID).Tables[0];
                if (dtSurvey.Rows.Count > 0)
                {
                    //IP受限
                    if (dtSurvey.Rows[0]["IsIPLimit"].ToBoolean())
                    {
                ...
                if (IsPass)
                {
                    ...
                    SurveyHelper.UpdateTotalCount(SurveyTableName, TopicID);
                    Result = "1";
                    #endregion
                }
                else
