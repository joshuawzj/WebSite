...
(function (factory) {
    ...
    STYLE_SETTING = 'style="max-width:160px;max-height:{height};"';
    ...
    defaultFileActionSettings = {
        ...
        dragIcon: '<i class="glyphicon entypo-menu"></i>',
        dragClass: 'text-info',
    ...
    tCaption = '<div tabindex="500" style="height:31px;" class="form-control file-caption {class}">\n' +
        ...
        _initPreviewActions: function () {
            ...
            self.$preview.find('.kv-file-remove').each(function () {
                ...
                if (isEmpty(vUrl) || vKey === undefined) {
                    handler($el, 'click', function () {
                        var text = $("#" + $("#" + self.id).attr("for")).val();
                        var rname = self.initialPreviewConfig[vKey].name;
                        var result = "";
                        if (text) {
                            for (var i = 0; i < self.initialPreviewConfig.length; i++) {
                                if (self.initialPreviewConfig[i].name != rname) {
                                    result += self.initialPreviewConfig[i].name + ",";
                                }
                            }
                        }
                        if (result) {
                            $("#" + $("#" + self.id).attr("for")).val(result.substring(0, result.length - 1));
                        }
                        else
                        {
                            $("#" + $("#" + self.id).attr("for")).val('');
                        }
                        var n, cap;
