...
(function (factory) {
    ...
    var NAMESPACE, MODAL_ID, STYLE_SETTING, OBJECT_PARAMS, DEFAULT_PREVIEW, objUrl, compare, isIE, handler,
        ...
        tPreview, tFileIcon, tClose, tCaption, tBtnDefault, tBtnLink, tBtnBrowse, tBtnPicker, tModalMain, tModal, tProgress, tSize,
        ...
        defaultPreviewSettings, defaultFileTypeSettings, isEmpty, isArray, ifSet, uniqId, htmlEncode, isBase64, replaceTags,
        cleanMemory, findFileName, checkFullScreen, toggleFullScreen, moveArray, FileInput;

    NAMESPACE = '.fileinput';
    MODAL_ID = 'kvFileinputModal';
    STYLE_SETTING = 'style="width:160px;height:{height};"';
    ...
    previewCache = {
        ...
        footer: function (id, i, isDisabled, size) {
            ...
            return footer.replace(/\{caption}/g, isBase64(caption, data.isCaption, false)).replace(/\{size}/g, data.getSize(size)).replace(/\{dataKey}/g, data.config[i].key)
    ...
    tPreview = '<div class="file-preview {class}">\n' +
        '    {close}' +
        '    <div class="{dropClass}">\n' +
        '    <div id={id} class="file-preview-thumbnails">\n' +
        '    </div>\n' +
    ...
    tBtnBrowse = '<div id={browseid} class="{css}" {status}>{icon} {label}</div>';
    tBtnPicker = '<div  class="btn btn-white" style="display:{display}"  onclick="openPicker(\'{url}\',\'{openTitle}\',this)" >{pickerLabel}</div>';
    ...
    tFooter = '<div class="file-thumbnail-footer">\n' +
        '    <div class="file-footer-caption hideCaption" title="{caption}"><span>{caption}</span>' +
        '     <input name="file_caption" class="form-control" value=\'{caption}\' data-key="{dataKey}"/>{size}</div >\n' +
        '    {progress} {actions}\n' +
        '</div>';
    ...
    tTagBef1 = tTagBef + '><div class="kv-file-content" style="background-image:url(\'{data}\');">\n';
    ...
    defaultPreviewZoomSettings = {
        image: { width: "auto", height: "auto", 'max-height': "100%", 'max-width': "100%" },
    ...
    isBase64 = function (val, isCaption, isEncode) {
        if (isCaption) {
            var b = new Base64();
            if (isEncode)
                return val == undefined ? "" :b.encode(val);
            else
                return val==undefined?"": htmlEncode(b.decode(val));
        }
        else
            return val;
    };
    ...
    FileInput = function (element, options) {  //1002
        var self = this;
    ...
    FileInput.prototype = {
            ...
            settings = {
                ...
                onSort: function (e) {
                    ...
                    for (var i = 0; i < self.initialPreviewConfig.length; i++) {
                        if (self.initialPreviewConfig[i]) {
                            result += self.initialPreviewConfig[i].name + (self.isCaption ? "?" : "*");      //图片路径    参考样例8888888 
                            if (self.isCaption)
                                result += self.initialPreviewConfig[i].caption + "*";   //文字描述    图片路径1?文字描述1*图片路径2?文字描述2
                        }
                    }
                    if (result) {
        ...
        _initPreview: function (isInit) {
            ...
            if (self.isCaption)  //是否显示文字描述输入
                out.content = out.content.replace(/hideCaption/g, "showCaption");
        ...
        _initPreviewActions: function () {
            var self = this, deleteExtraData = self.deleteExtraData || {},
                resetProgress = function () {
                    var files = self.$element.get(0).files, len;
                    if (!files)
                        len = 1;
                    else
                        len = files.length;

                    var hasFiles = self.isUploadable ? previewCache.count(self.id) : len;
            ...
            self.$preview.find('.kv-file-remove').each(function () { //1005
                var $el = $(this), vUrl = $el.data('url') || self.deleteUrl, vKey = $el.data('key');
                var $frame = $el.closest('.file-preview-frame'), cache = previewCache.data[self.id],
                    settings, params, index = $frame.data('fileindex'), config, extraData;
                index = parseInt(index.toString().replace('init_', ''));
                    ...
                    handler($el,
                        ...
                        function () {
                            ...
                            if (text) {
                                for (var i = 0; i < self.initialPreviewConfig.length; i++) {
                                    if (self.initialPreviewConfig[i]) {
                                        if (self.initialPreviewConfig[i].key != vKey) {
                                            result += self.initialPreviewConfig[i].name + (self.isCaption ? "?" : "*");      //图片路径    参考样例8888888 
                                            if (self.isCaption)
                                                result += self.initialPreviewConfig[i].caption + "*";   //文字描述    图片路径1?文字描述1*图片路径2?文字描述2
                                        }
            ...
            self.$preview.find('input[name="file_caption"]').each(function () { //1008 设置图片、文件描述
                var $el = $(this), vUrl = $el.data('url') || self.deleteUrl, vKey = $el.data('key');

                handler($el, 'blur', function () {
                    var caption = isBase64($(this).val(), self.isCaption, true);
                    var result = "";
                    for (var i = 0; i < self.initialPreviewConfig.length; i++) {
                        if (self.initialPreviewConfig[i]) {
                            if (self.initialPreviewConfig[i].key == vKey) {
                                self.initialPreviewConfig[i].caption = caption;
                            }
                            result += self.initialPreviewConfig[i].name + "?";      //图片路径    参考样例8888888 
                            result += self.initialPreviewConfig[i].caption + "*";   //文字描述    图片路径1?文字描述1*图片路径2?文字描述2
                        }
                    }
                    if (result) {
        ...
        _resetPreview: function () { //1006 重新加载预览图
        ...
        _ajaxSubmit: function (fnBefore, fnSuccess, fnComplete, fnError, previewId, index) {
            ...
            settings = $.extend(true, {}, {
                ...
                url: self.uploadUrl + escape(self.filenames[index]).replace(/\\+/g, '%2B').replace(/\\\"/g, '%22').replace(/\\'/g, '%27').replace(/\\/g, '% 2F'),
                type: 'POST',
                dataType: 'json',
                data: self.formdata,
                cache: false,
        ...
        _initUploadSuccess: function (out, $thumb, allFiles) {
            ...
            content = out.initialPreview || [_uploadFilesPath + out.response.Msg]; //上传路径在母版页面定义的
            config = out.initialPreviewConfig || [{ caption: isBase64(out.files[i].name, self.isCaption, true), size: out.files[i].size, name: out.response.Msg, key: self.initialPreview.length + 1 }];
            ...
            if ($thumb !== undefined) {
                if (!allFiles) {
                    if (self.overwriteInitial) {
                        previewCache.data[self.id].content = [];
                        previewCache.data[self.id].config = [];
                        previewCache.data[self.id].tags = [];
                    }
                ...
                setTimeout(function () {
                    if (isEmpty(data) || isEmpty(data.error)) {
                        if (self.showPreview) {
                            ...
                            $btnUpload.hide();
                            self._initUploadSuccess(outData, $thumb, allFiles);
                        }
                        if (self.isPic) {
                            self._raise('fileuploaded', [outData, $thumb.attr('id'), i]);
                        }
                        else {
                            self._raise('fileuploaded', [outData, '', i]);
                        }
                        if (!allFiles) {
                            self.updateStack(i, undefined);
                        } else {
        ...
        _generatePreviewTemplate: function (cat, data, fname, ftype, previewId, isError, size, frameClass, foot, ind) {
            ...
            if (cat === 'text' || cat === 'html') {
                ...
                content = tmplt.replace(/\{previewId}/g, previewId).replace(/\{caption}/g, isBase64(caption, self.isCaption, false))
                    .replace(/\{width}/g, config.width).replace(/\{height}/g, config.height)
                    .replace(/\{frameClass}/g, css).replace(/\{cat}/g, ftype)
                    .replace(/\{footer}/g, footer).replace(/\{fileindex}/g, ind)
                    .replace(/\{data}/g, sText).replace(/\{template}/g, cat)
                    .replace(/kv-file-content/g, self.isPic ? "kv-file-content contentCaption" : "kv-file-content");
            } else {
                content = tmplt.replace(/\{previewId}/g, previewId).replace(/\{caption}/g, isBase64(caption, self.isCaption, false))
        ...
        _previewFile: function (i, file, theFile, previewId, data) {
            ...
            if (chkTypes || chkMimes) {
                fname = isBase64(fname, self.isCaption, true);
                ...
                if ($preview.length && FileReader !== undefined) {
                    ...
                    reader.onloadend = function () {
                        msg = msgProgress.replace('{index}', i + 1).replace('{files}', numFiles)
                            .replace('{percent}', 100).replace('{name}', caption);
                        setTimeout(function () {
                            $status.html(msg);
                            self._updateFileDetails(numFiles);
                            readFile(i + 1);
        ...
        _setFileDropZoneTitle: function () {
            ...
            if ($zone.find('.file-preview-frame').length === 0 && isEmpty(self.defaultPreviewContent)) {
                $zone.prepend('<div id="' + self.id + '_drop" class="' + self.dropZoneTitleClass + '">' + title + '</div>');
            }
            self.$container.removeClass('file-input-new');
        ...
        _renderMain: function () {
            ...
            return self.mainTemplate.replace(/\{class}/g, self.mainClass) //+(!self.showBrowse && self.showCaption ? ' no-browse' : '')
                ...
                .replace(/\{picker}/g, self._renderButton('picker'))
                .replace(/\{id}/g, self.$element[0].id + "_preview")
                .replace(/\{browseid}/g, self.$element[0].id + "_browse");
        },
        _renderButton: function (type) {
            var self = this, tmplt = self._getLayoutTemplate('btnDefault'), css = self[type + 'Class'],
                title = self[type + 'Title'], icon = self[type + 'Icon'], label = self[type + 'Label'],
                status = self.isDisabled ? ' disabled' : '', btnType = 'button', url = self.pickerUrl,
                pickerLabel = self.isPic ? self['pickerPicLabel'] : self['pickerFileLabel'];
            switch (type) {
                case 'remove':
                    if (!self.showRemove) {
                        return '';
                ...
                case 'picker':
                    ...
                    if (!self.noPicker) //不是选择模式 就不显示 图片库、文件库按钮
                        tmplt = self._getLayoutTemplate('btnPicker');
                    else
                        tmplt = "";
                    break;
                default:
            ...
            return tmplt.replace('{type}', btnType).replace('{css}', css).replace('{title}', title)
                .replace('{status}', status).replace('{icon}', icon).replace('{label}', label).replace('{url}', url)
                .replace('{pickerLabel}', pickerLabel).replace('{openTitle}', pickerLabel).replace('{display}', self.noPicker ? "none" : "");
        },
        ...
        _renderFileFooter: function (caption, size, width, isError) {
            ...
            if (self.isUploadable) {
                out = template.replace(/\{actions}/g, self._renderFileActions(upl, rem, zoom, drg, false, false, false))
                    .replace(/\{caption}/g, isBase64(caption, self.isCaption, false)).replace(/\{size}/g, size).replace(/\{width}/g, width)
        ...
        refresh: function (options) { //1003 ie9上传和图片库选择图片 都是这更新预览图
            var self = this, $el = self.$element;
            ...
            if (options.isCustom) {
                ...
                if (initialPreview.length > 0 && initialPreviewConfig.length > 0) {
                    for (var i = 0; i < initialPreviewConfig.length; i++) {
                        initialPreviewConfig[i].key = self.initialPreviewConfig.length + 1;
                        initialPreviewConfig[i].caption = isBase64(initialPreviewConfig[i].caption, self.isCaption, true);
                        self.initialPreviewConfig.push(initialPreviewConfig[i]);
                        self.initialPreview.push(initialPreview[i]);
                    }
                }
                options.initialPreviewConfig = self.initialPreviewConfig;
                options.initialPreview = self.initialPreview;
    ...
    $.fn.fileinput = function (option) {  //1001
        ...
        this.each(function () {
            ...
            if (!data) {//&& data!=undefined) {
                ...
                if (isIE(9)) {  //ie9而且不是图片库、文件库、站点库页面 使用flash上传
                    // 初始化Web Uploader
                    var uploader = WebUploader.create({
                        // 自动上传。
                        auto: true,
                        // swf文件路径
                        swf: _sysPath + 'res/js/webuploader/Uploader.swf',
                        // 文件接收服务端。ploadUrl: ""
                        server: opts.uploadUrl + "&browser=ie9",
                        ...
                        // 只允许选择文件，可选。
                        accept: {
                            title: opts.isPic ? 'Images' : 'Files',
                            extensions: $(this).attr('accept').replace(/\./g, ""),
                            mimeTypes: opts.isPic ? 'image/*' : 'file/*'
                        },
                        controlId: this.id,  //上传控件id
                        isRefresh: opts.noPicker,  //只有图片库、文件库、站点库页面刷新
                        fileType: opts.initialPreviewFileType  //是否图片上传
                    });
                    // 文件上传成功，给item添加成功class, 用样式标记上传成功。
                    uploader.on('uploadSuccess', function (file, response) {
                        var filePath = response.Msg;
                        var controlId = $("#" + this.options.controlId);
                        var initialPreview = [];
                        var initialPreviewConfig = [];
                        initialPreview.push(_uploadFilesPath + filePath);
                        initialPreviewConfig.push({ caption: file.name, size: file.size, name: filePath, key: parseInt(Math.random() * 1000) });
                        controlId.fileinput('refresh', { isCustom: true, initialPreview: initialPreview, initialPreviewConfig: initialPreviewConfig });

                        if (this.options.isRefresh) { //只有图片库、文件库、站点库页面刷新
                            var path = $("#txtPath").val();
                            var fileType = this.options.fileType == "image" ? "UploadImages" : "UploadFiles";
                            $table.bootstrapTable('refresh', { url: _sysPath + "Handler/Extension/" + fileType + ".aspx?_action=GetList&path=" + path });
                        }
                        else   //栏目上传 需要校验必填
                            formRequiredValidator($("#" + this.options.controlId).attr("for"), true); //校验必填项的方法 
                    });

                    $("#" + this.id).data('fileinput', data);
                }
                else
    ...
    $.fn.fileinputLocales.en = {
        ...
        pickerPicLabel: 'Images',
        pickerFileLabel: 'Files',
        msgNo: 'No',
