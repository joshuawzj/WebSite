(function ($) {
    ...
    BootstrapTable.prototype.initToolbar = function () {
        ...
        if (this.options.showPrint) {
            ...
            if (!$print.length) {
                $print = $([
                    '<button class="bs-print btn btn-default' + sprintf(' btn-%s"', this.options.iconSize) + '" name="print" title="' + this.options.formatPrint() +'" type="button">',
                    ...
                    function buildTable(data,columns) {
                        ...
                        for(var h = 0; h < columns.length; h++) {
                            if(!columns[h].printIgnore) {
                                out += ("<th>" + (columns[h].title ? columns[h].title : '主键') + "</th>");
                            }
                        }
                        out += "</tr></thead><tbody>";
