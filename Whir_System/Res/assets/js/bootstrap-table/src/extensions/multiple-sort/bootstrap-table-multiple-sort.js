            ...
            that.$sortModal.off('click', '.btn-primary').on('click', '.btn-primary', function() {
                ...
                if (results.length > 0) {
                    ...
                    if (that.options.sidePagination === 'server') {
                        ...
                        for (var j = 0; j < that.options.sortPriority.length; j++) {
                            var th = that.$header.find('th[data-field=CreateDate]');
                            th.find('.sortable').removeClass('desc asc').addClass(that.options.sortPriority[j].sortOrder ? that.options.sortPriority[j].sortOrder : 'both');
                        }
