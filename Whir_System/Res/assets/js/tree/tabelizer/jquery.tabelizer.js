	...
	self.init = function(){
		...
		self.caller.find('tr').each( function(){
			...
			if (!$row.hasClass('header')){
				...
				$row.addClass(rowClass); //apply labels around the first column text, so that we can add in the controls and expander image
				$firstCol = $($row.children('td')[0]);
			    var firstColVal = '<div class="tree-key">' + $firstCol.html() + '</div>';
				var parentLevels = 0;
				...
				levelLines += '</div>'; //Add the expanded class, which through css adds in the arrow image

			    //判断后面有没有下级
				if ($row.next("[data-level='" + Number(currentLevel + 1) + "']").length>0) {
			        $firstCol.html(levelLines + ' <div class="expander"></div> ' + firstColVal);
			    } else {
				    $firstCol.html(levelLines + ' <div class="placeholder"></div> ' + firstColVal);
			    }
