<%@ Page Title="" Language="C#" MasterPageFile="~/whir_system/master/SystemMasterPage.master"
    AutoEventWireup="true" CodeFile="subjectformlist.aspx.cs" Inherits="whir_system_module_subject_subjectformlist" %>
        ...
        <div class="panel">
            <div class="panel-body">
                <ul class="nav nav-tabs">
                    <li><a href="subjectcolumnlist.aspx?subjecttypeid=<%= SubjectTypeId %>&subjectclassid=<%= SubjectClassId %>" aria-expanded="true"><%="栏目管理".ToLang()%></a></li>
                    ...
                    <li <%=cla %>>
                        <a href="SubjectFormlist.aspx?subjecttypeid=<%= SubjectTypeId %>&subjectclassid=<%= SubjectClassId %>&columnid=<%= item.ColumnId %>"><%=item.ColumnName%>-表单</a></li>
                    <%  } %>
                </ul>
                <div class="space15"></div>
                <div class="actions btn-group">

                    <a class="btn btn-white" href="SubjectForm_Edit.aspx?subjecttypeid=<%= SubjectTypeId %>&subjectclassid=<%= SubjectClassId %>&columnid=<%= ColumnId %>">
                        <%="添加表单输入项".ToLang()%></a>
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0" class="controller table table-bordered table-noPadding sortTable ui-sortable">
                    <tr>
                        <th>
                            <div class="th-inner ">
                                <input type="checkbox" id="cbxAll" name="cb_Top" /></div>
                        </th>
                        <th>
                            <%="排序".ToLang()%>
                        </th>
                        ...
                        <th width="<%=LanguageHelper.GetSplitValue("322px|322px|342px") %>">
                            <%="操作".ToLang()%>
                        </th>
                    </tr>
                        ...
                        <asp:Repeater ID="rptFormList" runat="server">
                            <ItemTemplate>
                                <tr sort="true">
                                    ...
                                    <td sorttxt="true">
                                        <input type="text" style="width: 133px" id="txtSort" class="form-control form-search-control"
                                            value='<%# Eval("Sort") %>' formid='<%# Eval("FormId") %>'></input>
                                    </td>
                                    ...
                                    <td>
                                        <div class="btn-group">
                                           <a class="btn btn-sm btn-white aEdit" href='SubjectForm_Edit.aspx?subjecttypeid=<%= SubjectTypeId %>&subjectclassid=<%= SubjectClassId %>&columnid=<%= ColumnId %>&formid=<%# Eval("FormId") %>'> <%="编辑".ToLang()%></a>
                                         ...
                                         <asp:PlaceHolder ID="lbnDelete" runat="server">
                                            <a name="lbnDelete" class="btn btn-sm text-danger border-danger" href="javascript:;" onclick="Delete(<%# Eval("FormId") %>)"><%="删除".ToLang()%></a>
                                         </asp:PlaceHolder>
                                      </div>
                                    </td>
                                </tr>
                ...
                <div class="operate_foot">
                    ...
                    <a id="lbnDelete" class="btn btn-sm text-danger border-danger" href="javascript:;"><%="批量删除".ToLang()%></a>
                </div>
            </div>
        </div>
    </div>
        ...
        $(document)
            .ready(function () {
                ...
                $("#tbdFormList a[name=ListShow]")
                 ...
                 //列表显示/掩藏
                 $("#tbdFormList a[name=ListShow],a[name=SearchShow]")
                     .click(function () {
                             ...
                             if (response.Status == true) {
                                 ...
                                 if (isList) {
                                     if (isshow === "false") {
                                         ...
                                         $(el).html("<%="列表显示".ToLang()%>");
                                         $(el).addClass("btn btn-sm btn-white");
