<%@ Control Language="C#" AutoEventWireup="true" CodeFile="Jurisdiction_column_top.ascx.cs"
    Inherits="whir_system_module_developer_Jurisdiction_column_top" %>
...
<div class="panel panel-default">
    ...
    <div class="panel-body">
        <div class="row">
            <asp:DataList ID="rptList" runat="server" RepeatColumns="2" RepeatDirection="Horizontal"
                Width="100%">
                <ItemTemplate>
                    <div class="col-sm-12">
                        <b><%#Eval("SiteName").ToStr()%>：</b>
                        <a href="Jurisdiction_column.aspx?siteid=<%#Eval("SiteId").ToStr()%>&type=0" id="<%#Eval("SiteId").ToStr()%>0"><%="站点栏目".ToLang() %></a>
                        <a href="Jurisdiction_subject.aspx?siteid=<%#Eval("SiteId").ToStr()%>&type=1" id="<%#Eval("SiteId").ToStr()%>1"><%="子站栏目".ToLang() %></a>
                        <a href="Jurisdiction_subject.aspx?siteid=<%#Eval("SiteId").ToStr()%>&type=2" id="<%#Eval("SiteId").ToStr()%>2"><%="专题栏目".ToLang() %></a>
                    </div>
                </ItemTemplate>
            </asp:DataList>
...
<script>
    ...
    function goUrl(o) {
        if ($(o).val() == "")
            return;
        location.href = "Jurisdiction_subject.aspx?siteid=<%=SiteId%>&type=<%=SelectType%>&classid=" + classId + "&subjectId=" + $(o).val();
    }
