<%@ Page Title="" Language="C#" MasterPageFile="~/whir_system/master/SystemMasterPage.master" AutoEventWireup="true" CodeFile="UploadFiles.aspx.cs" Inherits="Whir_System_Module_Extension_UploadFiles" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    <form id="Form1" runat="server" method="post" action="../../ajax/extension/uploadfiles.aspx" enctype="multipart/form-data">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="input-group uploadimages-input-group">
                                                <%if (!IsCurrentRoleMenuRes("312"))
                                                  { %>
                                                <span class="input-group-addon btn btn-info"><%="当前文件夹".ToLang()%>
                                                </span>
                                                <%} %>
                                        ...
                                        <div class="col-md-6">
                                            <div class="input-group uploadimages-input-group">
                                                ...
                                                <%} %>
                                                <a class="input-group-addon btn btn-white" onclick="reload();"><%="刷新".ToLang()%>
                                                </a>
                                                <%if (IsCurrentRoleMenuRes("315"))
                                                  { %>
    ...
    <script type="text/javascript">
        ...
        function regeditUploadFile() {
            $("#txt_file")
                .fileinput({
                    ...
                    uploadUrl: "<%=SysPath%>Ajax/Extension/uploadfiles.aspx?FormId=0&savePath=" + currentpath + "&fileName=",
                    previewFileType: "any",
        ...
        function getTreeJson() {
            whir.ajax.post('<%= SysPath%>Handler/Extension/UploadFiles.aspx', {
                data: {
        ...
        function bindTree(treeJson) {
            ...
            $('#folderList').treeview1({
                ...
                onNodeSelected: function (event, node) {                              //切换文件夹
                    ...
                    $table.bootstrapTable('refresh', { url: "<%=SysPath%>Handler/Extension/UploadFiles.aspx?_action=GetList&path=" + encodeURIComponent(path) });
        ...
        function initTable() {
            $table.bootstrapTable({
                url: "<%=SysPath%>Handler/Extension/UploadFiles.aspx?_action=GetList&path=<%=UploadFilePath %>",
                dataType: "json",
                pagination: true, //分页
                ...
                onPageChange: function () {
                    ...
                    var key = $("#key").val();
                    if (key == "")
                    	$table.bootstrapTable('refreshOptions', { url: "<%=SysPath%>Handler/Extension/UploadFiles.aspx?_action=GetList&path=" + path });
                    else
                        $table.bootstrapTable('refreshOptions', { url: "<%=SysPath%>Handler/Extension/UploadFiles.aspx?_action=GetList&key=" + key + "&path=" + path });
               },
        ...
        function GetOperation(value, row, index) {
            ...
            if (row.Type == "File") {
                <%if (IsCurrentRoleMenuRes("316"))
                  { %>
                html += '<a class="btn btn-xs btn-white" href="Download.aspx?path=<%=UploadFilePath %>&name=' + encodeURIComponent(row.FullName) + '" target="_blank"><%="下载".ToLang()%></a>';
                <%}%>
            }
             <%if (IsCurrentRoleMenuRes("315"))
               { %>
        ...
        function reload() {
            var path = $("#txtPath").val();
            $table.bootstrapTable('refresh', { url: "<%=SysPath%>Handler/Extension/UploadFiles.aspx?_action=GetList&path=" + path });
            whir.loading.remove();
        }
        ...
        function searchFile() {
            var path = $("#txtPath").val();
            var key = $("#key").val();
            $table.bootstrapTable('refresh', { url: "<%=SysPath%>Handler/Extension/UploadFiles.aspx?_action=GetList&key=" + key + "&path=" + path });
