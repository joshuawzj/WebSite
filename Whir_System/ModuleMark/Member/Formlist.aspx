...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    <div class="content-wrap">
        ...
        <div class="panel">
            <div class="panel-heading"><%="会员模型".ToLang()%></div>
            <div class="panel-body">
                <div class="actions btn-group pull-left">
                    <a class="btn btn-white" href="Form_Edit.aspx?columnid=<%=ColumnId%>"><%="添加表单输入项".ToLang()%></a> 
                    <a class="btn btn-white" href="javascript:whir.dialog.frame('<%="批量添加表单输入项".ToLang()%>', '<%=SysPath%>Module/Column/Form_BatchAdd.aspx?columnid=<%=ColumnId%>',null,700,650);"><%="批量添加表单输入项".ToLang()%></a>
                </div>
                     ...
                     <% foreach (var item in ListColumnP)
                        {
                          ...
                          %>
                            <% }%>
                             ...
                             <li <%=cla%>><a href="Formlist.aspx?columnid=<%=item.ColumnId%>"><%=item.ColumnName.ToLang()%></a></li>
                      <%  }%>
                </ul>--%>
                <div  class="All_list All_list-first" style="clear:both;">
                <div class="tableCategory-table-body">
                <table width="100%" class="controller table table-bordered table-noPadding sortTable ui-sortable">
                    <tr >
                        <th >
                        ...
                        <th>Id</th>
                        <th><%="数据库字段名".ToLang()%></th>
                        <th width="120px"><%="表单别名".ToLang()%></th>
                        <th width="120px"><%="表单类型".ToLang()%></th>
                        <th><%="是否系统字段".ToLang()%></th>
                        <th><%="是否隐藏".ToLang()%></th>
                        <th >
                            <%="操作".ToLang()%>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $("#tbdFormList a[name=ListShow]").each(function() {
                ...
                if (isListShow == "true") {
                    ...
                    $(this).addClass("btn btn-sm text-success border-normal");
                }
            });
        ...
        $("#tbdFormList a[name=ListShow],a[name=SearchShow]").click(function() {
            ...
            var isshow = isList ? $(this).attr("IsListShow").toLowerCase() : $(this).attr("IsSearchShow").toLowerCase();
            var el = this;
            whir.ajax.post("<%=SysPath%>Handler/Module/Column/Form.aspx", {
                    data: {
                    ...
                    success: function(response) {
                        ...
                        if (response.Status == true) {
                            ...
                            if (isList) {
                                if (isshow == "false") {
                                    $(el).html("<%="列表隐藏".ToLang()%>");
                                    $(el).addClass("btn btn-sm text-danger border-normal");
                                    $(el).attr("IsListShow", "true");
                                } else {
                                    $(el).html("<%="列表显示".ToLang()%>");
                                    $(el).addClass("btn btn-sm text-success border-normal");
